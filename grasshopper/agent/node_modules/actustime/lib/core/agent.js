...
Agent.prototype.init = function(opts) {
  ...
  // Registered accounts.
  self.accountKey = opts.accountKey || 'baejangeeebaejangeeebaejangeeebaejangeee';
  if(!self.accountKey) {
...
//  self.sessionId = 'pro:' + self.accountKey + ':' + self.utils.sha1(self.appName);
  self.sessionId = self.appName;

  self.interval = opts.interval*1000 || 60000;
  ...
  self.features.hostMetrics = 
    opts.features.hostMetrics || !os.hostname().match(/^[a-f0-9\-]{38}$/);
  ...
  self.logger.log(self.features.hostMetrics);
...
Agent.prototype.loadProbes = function() {
  ...
  probeCons.forEach(function(probeCon) {
    ...
    probe.packages.forEach(function(pkg) {
      self.logger.log('DEBUG: probeCons '+ pkg);
  ...
  self.proxy.after(module.__proto__, 'require', function(obj, args, ret) {
    ...
    self.logger.log('DEBUG: probe '+args[0]);
    if(probe) {
      self.logger.log('DEBUG: attached '+ret);
      self.logger.log(ret);
      probe.attach(ret);
    }
  });
